...
/**
 * 地形,内含障碍物
 * 障碍物由一个个的点构成,点的x,y坐标标明该障碍物位于地形的位置
...
public class Ground {
    ...
    /**
     * 保存障碍物的每一点的坐标和类型,每个障碍物点由坐标和类型构成
     * [i][0]: 障碍物点的x坐标
     * [i][1]: 障碍物点的y坐标
     * [i][2]: 障碍物点的类型
     * 类型主要用于获取障碍物点的颜色, 默认类型为-1, 将形状变为障碍物时将由形状的类型决定
     */
    private int[][] points;
    ...
    /**
     * 当一个形状下落到底的位置,
     * 一个形状下落到底,x位置不变,只需要计算下落到底的y应该在的位置
     * @param shape
     * @return
     */
    public Shape dropDownShape(Shape shape){
        int width = shape.getWidth();
        int left = shape.getLeft();
        int[][] shapePoints = shape.getPoints();
        int shapeLength = shapePoints.length;

        //类型为0和13的线性方块,实际宽度需要减去offset值
        int type = shape.getType();
        if ((type == 0 || type == 13) && shape.getStatus() == 2){
            int offset = (shape.getPoints().length - 1) / 2;
            width -= offset;
        }
        //离障碍物最近的几个点
        int[][] nearPoints = new int[width][2];
        for (int i = left; i < left + width; i++){
            nearPoints[i - left][0] = i;
            for (int j = 0; j < shapeLength; j++){
                if (shapePoints[j][0] == i){
                    if(nearPoints[i - left][1] == 0)
                        nearPoints[i - left][1] = shapePoints[j][1];
                    for (int k = j + 1; k < shapeLength; k++){
                        if (shapePoints[k][0] == i && shapePoints[k][1] > nearPoints[i - left][1]){
                            nearPoints[i - left][1] = shapePoints[k][1];
                        }
                    }
                    break;
                }
        ...
        System.out.println("------shapePoints:");
        System.out.println("width:"+width);
        for (int i = 0; i < shapeLength; i++){
            System.out.println("x:"+shapePoints[i][0]+" y:"+shapePoints[i][1]);
        }

        System.out.println("------nearPoints:");
        for (int i = 0; i < width; i++){
            System.out.println("x:"+nearPoints[i][0]+" y:"+nearPoints[i][1]);
        }

        //离形状最近的障碍物
        int[][] nearObstaclePoints = new int[width][2];
        int obestaclesLength = points == null ? 0 : points.length;
        if (obestaclesLength == 0)
            System.out.println("obstacle null----");
        for (int i = left; i < left + width; i++){
            nearObstaclePoints[i - left][0] = i;
            nearObstaclePoints[i - left][1] = this.getHeight();
            for (int j = 0; j < obestaclesLength; j++){
                if (points[j][0] == i && points[j][1] > nearPoints[i - left][1]){
                    if(points[j][1] < nearObstaclePoints[i - left][1])
                        nearObstaclePoints[i - left][1] = points[j][1];
                    for (int k = j + 1; k < obestaclesLength; k++){
                        if (points[k][0] == i && points[k][1] < nearObstaclePoints[i - left][1] && points[k][1] > 0){
                            nearObstaclePoints[i - left][1] = points[k][1];
                        }
                    }
                    break;
                }
        ...
        System.out.println("------nearObstaclePoints:");
        for (int i = 0; i < width; i++){
            System.out.println("x:"+nearObstaclePoints[i][0]+" y:"+nearObstaclePoints[i][1]);
        }

        int[] nearDistances = new int[width];
        for (int i = 0; i < width; i++){
            nearDistances[i] = nearObstaclePoints[i][1] - nearPoints[i][1];
        }
        int nearest = nearDistances[0];
        for (int i = 1; i < width; i++){
            if (nearest > nearDistances[i]) nearest = nearDistances[i];
        }
        Shape dropedShape = (Shape)shape.clone();
        dropedShape.setTop(dropedShape.getTop() + nearest - 1);
